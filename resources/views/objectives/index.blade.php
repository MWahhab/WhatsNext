...
<script>
    window.onload = function () {
        ...
        //assignInitialViewingDate();
    };

    // function assignInitialViewingDate() {
    //     let currentDate   = new Date();
    //     let formattedDate = currentDate.toLocaleDateString("en-GB");
    //
    //     document.getElementById("week-selector").value = formattedDate;
    // }
...
`;
    ...
    function setNewVieww(data) {
        let viewContainer = document.getElementById("view-container");

        viewContainer.innerHTML = "";
        for (const [day, dateWithObjectives] of Object.entries(data)) {
            console.log("day:", day);
            console.log("dateWithObjectives:", dateWithObjectives);

            let outerDayCardEle = document.createElement("div");
            let innerDayCardEle = document.createElement("div");

            outerDayCardEle.className =
                "img-box";
            outerDayCardEle.setAttribute("id", `${day}-card`);
            ...
            console.log("inenr day caard ele:", innerDayCardEle);
            ...
            let flowRootDiv = document.createElement("div");

            flowRootDiv.className = "flow-root";

            let ulEle = document.createElement("ul");

            ulEle.setAttribute("id", `${dateWithObjectives['date']}-card`)
            ulEle.setAttribute("role", "list");
            ulEle.className = "divide-y divide-gray-200 dark:divide-gray-700";

            // innerDayCardEle.insertAdjacentHTML('beforeend', `
            // <div class="flow-root">
            //     <ul id="${dateWithObjectives['date']}-card" role="list" class="divide-y divide-gray-200 dark:divide-gray-700">
            // `);

            Object.keys(dateWithObjectives['objectives']).forEach(key => {
                let objective = dateWithObjectives["objectives"][key];
...
`;
                console.log("listing ele: ", listingEle);

                ulEle.append(listingEle);
            })

            flowRootDiv.append(ulEle);

            console.log("flow root div: ", flowRootDiv);

            innerDayCardEle.append(flowRootDiv);

            console.log("innerdayele: ", flowRootDiv);

            let endOfOuterCard = `
<h3 id="${dateWithObjectives['date']}-heading" class="headings">${day.charAt(0).toUpperCase() + day.slice(1)}
            ...
            outerDayCardEle.append(innerDayCardEle);
            outerDayCardEle.insertAdjacentHTML('beforeend', endOfOuterCard);

            console.log("innerDayCard ele: ", innerDayCardEle);

            viewContainer.append(outerDayCardEle);

            console.log("view card: ", viewContainer)
